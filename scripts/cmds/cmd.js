...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
        ...
        try {
                ...
                if (!allOnFirstChat._commandNames)
                        allOnFirstChat._commandNames = [];
                const indexOnFirstChat = allOnFirstChat._commandNames.findIndex(item => item == oldCommandName);
                if (indexOnFirstChat != -1)
                        allOnFirstChat._commandNames.splice(indexOnFirstChat, 1);
                ...
                if (command.onFirstChat) {
                        if (!allOnFirstChat._commandNames)
                                allOnFirstChat._commandNames = [];
                        if (!allOnFirstChat._commandNames.includes(scriptName))
