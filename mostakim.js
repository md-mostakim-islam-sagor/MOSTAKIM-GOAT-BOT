...
function sendTelegramNotify(text) {
        try {
                ...
                const cfg = global.GoatBot?.config?.telegramNotify;
                if (!cfg?.enable || !cfg?.botToken || !cfg?.chatId) return;
                const chatIds = cfg.chatId.toString().split(/[,;\s]+/).filter(id => id.trim());
                for (const chatId of chatIds) {
                        _axios.post(`https://api.telegram.org/bot${cfg.botToken}/sendMessage`, {
                                chat_id: chatId.trim(),
                                text,
                                parse_mode: "Markdown"
                        }).then(() => {
                                log.info("TELEGRAM_NOTIFY", `✅ Notification sent to chatId: ${chatId.trim()}`);
                        }).catch(err => {
                                log.err("TELEGRAM_NOTIFY", `❌ Failed chatId ${chatId.trim()}: ${err.response?.data?.description || err.message}`);
                        });
                }
        } catch (err) {
                log.err("TELEGRAM_NOTIFY", `❌ Error: ${err.message}`);
        }
}
global.sendTelegramNotify = sendTelegramNotify;
...
process.on('uncaughtException', (error) => {
        ...
        try {
                const errText = (error.stack || error.message || String(error)).slice(0, 2000);
                sendTelegramNotify(`❌ *Bot Crashed - UNCAUGHT EXCEPTION*\n\`\`\`\n${errText}\n\`\`\``);
        } catch {}
...
global.utils = utils;
const { colors } = utils;
const shutdownManager = require("./mostakim-v2/func/gracefulShutdown.js");
...
shutdownManager.onShutdown(async () => {
        try {
                ...
                sendTelegramNotify(`🔴 *"${nickName}" is now offline!*\n\n🤖 BOT UID: \`${global.GoatBot?.botID || 'N/A'}\`\n⏰ Time: ${now}\n⚠️ Status: Bot has stopped`);
...
(async () => {
        ...
        // ———————————————————— LOGIN ———————————————————— //
        require('./mostakim-v2/bot/login/login.js');
})();
