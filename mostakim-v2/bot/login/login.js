// set bash title
process.stdout.write("\x1b]2;Goat Bot V2 - Made by MOSTAKIM\x1b\x5c");
...
const { login } = defaultRequire("@mostakim-v2/fca-unofficial");
...
let subTitle = `MOSTAKIM GOAT BOT V2@${currentVersion} - A simple Bot chat messenger use personal account`;
const subTitleArray = [];
...
const modified = ("Modified by MD MOSTAKIM ISLAM SAGOR");
const srcUrl = ("Source code: https://github.com/md-mostakim-islam-sagor/MOSTAKIM-GOAT-BOT");
...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                const validFcaOptions = ["online", "selfListen", "selfListenEvent", "listenEvents", "listenTyping", "updatePresence", "autoMarkRead", "autoReconnect", "forceLogin", "userAgent", "logLevel", "proxy", "pageID"];
                const rawFcaOpts = global.GoatBot.config.optionsFca || {};
                const filteredFcaOpts = Object.fromEntries(Object.entries(rawFcaOpts).filter(([k]) => validFcaOptions.includes(k)));
                login({ appState }, filteredFcaOpts, async function (error, api) {
                        ...
                        try {
                                if (typeof api.getHealthStatus === "function") {
                                        ...
                                        log.info("FCA", `Health status — MQTT: ${health.mqtt?.connected ? "connected" : "disconnected"}`);
                                }
                        } catch (e) {
                                // health check not supported in this FCA version
                        }
                        // ———————————————————— GBAN ————————————————————— //
                        let dataGban;
                                                ...
                                                if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/cmds/${filename}`)) {
                                                        try {
                                                                ...
                                                                if (infoLoad.status == "success")
                                                                        ...
                                                                        try { if (global.sendTelegramNotify) global.sendTelegramNotify(`⚠️ *Command Load Failed*\n\`${filename}.js\`\n\`\`\`\n${(infoLoad.error?.message || infoLoad.error || "Unknown error").toString().slice(0, 800)}\n\`\`\``); } catch {}
                                                                }
                                                        }
                                                        catch (err) {
                                                ...
                                                if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/events/${filename}`)) {
                                                        try {
                                                                ...
                                                                if (infoLoad.status == "success")
                                                                        ...
                                                                        try { if (global.sendTelegramNotify) global.sendTelegramNotify(`⚠️ *Event Load Failed*\n\`${filename}.js\`\n\`\`\`\n${(infoLoad.error?.message || infoLoad.error || "Unknown error").toString().slice(0, 800)}\n\`\`\``); } catch {}
                                                                }
                                                        }
                                                        catch (err) {
                        ...
                        for (const line of activeBanner) {
                                const colored = gradient("#FA8BFF", "#2BD2FF", "#2BFF88")(line);
                                centerText(colored, line.length);
                        }
                        const readyLine1 = "✅  BOT IS FULLY ACTIVE & LISTENING  ✅";
                        const readyLine2 = `🤖  MOSTAKIM GOAT BOT V2 @${currentVersion}  🤖`;
                        const readyLine3 = `⏱   LOAD TIME: ${convertTime(Date.now() - global.GoatBot.startTime)}   ⏱`;
                        centerText(gradient("#f5af19", "#f12711")(readyLine1), readyLine1.length);
                        centerText(gradient("#9F98E8", "#AFF6CF")(readyLine2), readyLine2.length);
                        centerText(gradient("#9F98E8", "#AFF6CF")(readyLine3), readyLine3.length);
                        console.log(gradient("#f5af19", "#f12711")(createLine(null, true)));
                        console.log();
                        // ————————————————————————————————————————————————— //
                        ...
                        if (restartListenMqtt.enable == true) {
                                if (restartListenMqtt.logNoti == true) {
                                        ...
                                        try {
                                                if (global.sendTelegramNotify) {
                                                        const now = new Date().toLocaleString("en-BD", { timeZone: "Asia/Dhaka" });
                                                        const nickName = global.GoatBot.config.nickNameBot || 'MOSTAKIM GOAT BOT';
                                                        const adminUIDs = (global.GoatBot.config.adminBot || []).join(', ') || 'N/A';
                                                        const platform = (() => {
                                                                ...
                                                                if (process.env.CYCLIC_APP) return 'Cyclic';
                                                                return 'Local/VPS';
                                                        })();
                                                        global.sendTelegramNotify(
                                                                `⚙️ *"${nickName}" is now online!*\n\n` +
                                                                `🔰 ADMIN UID: \`${adminUIDs}\`\n` +
                                                                `🤖 BOT UID: \`${botID}\`\n` +
                                                                `🔧 Prefix: \`${global.GoatBot.config.prefix}\`\n` +
                                                                `♻️ Status: Running & receiving messages\n` +
                                                                `⏰ Started: ${now}\n` +
                                                                `🌐 Platform: ${platform}\n` +
