        ...
        return async function (event, message) {
                ...
                */
                ...
                async function onStart() {
                        ...
                        // —————————— CHECK MUTED USER (reactBy mute) —————————— //
                        if (isGroup && role < 2) {
                                const mutedUsers = threadData?.data?.mutedUsers || [];
                                if (mutedUsers.includes(String(senderID)))
                                        return;
                        }
                ...
                */
                async function onReaction() {
                        ...
                        /*
                         +------------------------------------------------+
                         |                   REACT BY                      |
                         | Admins react to messages to moderate them       |
                         +------------------------------------------------+
                        */
                        if (reaction && isGroup && role >= 4) {
                                const reactByConfig = config.reactBy || {};
                                const deleteEmojis = reactByConfig.delete || [];
                                const kickEmojis = reactByConfig.kick || [];
                                const warnEmojis = reactByConfig.warn || [];
                                const muteEmojis = reactByConfig.mute || [];
                                const adduserEmojis = reactByConfig.adduser || [];

                                // Target = author of the original message being reacted to
                                const targetID = event.messageAuthorID || event.reactTarget?.authorID;

                                // ——— DELETE: unsend the reacted-to message ———
                                if (deleteEmojis.includes(reaction)) {
                                        try {
                                                await api.unsendMessage(messageID, threadID);
                                                if (Reaction) onReaction.delete(messageID);
                                                log.info("REACT_BY", `[DELETE] ${senderID} deleted message ${messageID} in ${threadID}`);
                                        } catch (err) {
                                                log.err("REACT_BY", "Failed to delete message", err);
                                        }
                                        return;
                                }

                                // ——— Actions below require knowing the target user ———
                                if (targetID && targetID !== String(global.GoatBot.botID)) {
                                        const targetRole = getRole(threadData, targetID);

                                        // ——— KICK: remove the message author from group ———
                                        if (kickEmojis.includes(reaction)) {
                                                if (targetRole >= role && role < 2) {
                                                        // Cannot kick equal or higher role unless bot admin
                                                } else {
                                                        try {
                                                                const targetName = await usersData.getName(targetID).catch(() => targetID);
                                                                await api.removeUserFromGroup(targetID, threadID);
                                                                await message.reply(`👢 ${targetName} has been kicked by reaction moderation.`);
                                                                log.info("REACT_BY", `[KICK] ${senderID} kicked ${targetID} in ${threadID}`);
                                                        } catch (err) {
                                                                log.err("REACT_BY", "Failed to kick user", err);
                                                                await message.reply("⚠️ Failed to kick. Make sure bot is admin.");
                                                        }
                                                }
                                                return;
                                        }

                                        // ——— WARN: add warn to threadData (consistent with warn command) ———
                                        if (warnEmojis.includes(reaction)) {
                                                if (targetRole >= role && role < 2) {
                                                        // Cannot warn equal or higher role
                                                } else {
                                                        try {
                                                                const warnList = await threadsData.get(threadID, "data.warn", []);
                                                                const targetName = await usersData.getName(targetID).catch(() => targetID);
                                                                const dateTime = getTime("DD/MM/YYYY hh:mm:ss");
                                                                const dataWarnOfUser = warnList.find(item => item.uid == targetID);
                                                                if (!dataWarnOfUser) {
                                                                        warnList.push({
                                                                                uid: targetID,
                                                                                list: [{ reason: "Reaction warn", dateTime, warnBy: senderID }]
                                                                        });
                                                                } else {
                                                                        dataWarnOfUser.list.push({ reason: "Reaction warn", dateTime, warnBy: senderID });
                                                                }
                                                                await threadsData.set(threadID, warnList, "data.warn");
                                                                const times = dataWarnOfUser ? dataWarnOfUser.list.length : 1;
                                                                if (times >= 3) {
                                                                        await message.reply(`⚠️ ${targetName} has been warned ${times} times and will be kicked!`);
                                                                        await api.removeUserFromGroup(targetID, threadID);
                                                                } else {
                                                                        await message.reply(`⚠️ ${targetName} has been warned (${times}/3). Reason: Reaction warn.`);
                                                                }
                                                                log.info("REACT_BY", `[WARN] ${senderID} warned ${targetID} (${times}/3) in ${threadID}`);
                                                        } catch (err) {
                                                                log.err("REACT_BY", "Failed to warn user", err);
                                                        }
                                                }
                                                return;
                                        }

                                        // ——— MUTE: add user to muted list in threadData ———
                                        if (muteEmojis.includes(reaction)) {
                                                if (targetRole >= role && role < 2) {
                                                        // Cannot mute equal or higher role
                                                } else {
                                                        try {
                                                                const mutedUsers = await threadsData.get(threadID, "data.mutedUsers", []);
                                                                const targetName = await usersData.getName(targetID).catch(() => targetID);
                                                                if (!mutedUsers.includes(String(targetID))) {
                                                                        mutedUsers.push(String(targetID));
                                                                        await threadsData.set(threadID, mutedUsers, "data.mutedUsers");
                                                                        await message.reply(`🔇 ${targetName} has been muted. They cannot use bot commands.`);
                                                                        log.info("REACT_BY", `[MUTE] ${senderID} muted ${targetID} in ${threadID}`);
                                                                } else {
                                                                        await message.reply(`ℹ️ ${targetName} is already muted.`);
                                                                }
                                                        } catch (err) {
                                                                log.err("REACT_BY", "Failed to mute user", err);
                                                        }
                                                }
                                                return;
                                        }

                                        // ——— ADDUSER: unmute (re-add) a muted user ———
                                        if (adduserEmojis.includes(reaction)) {
                                                try {
                                                        const mutedUsers = await threadsData.get(threadID, "data.mutedUsers", []);
                                                        const targetName = await usersData.getName(targetID).catch(() => targetID);
                                                        const idx = mutedUsers.indexOf(String(targetID));
                                                        if (idx !== -1) {
                                                                mutedUsers.splice(idx, 1);
                                                                await threadsData.set(threadID, mutedUsers, "data.mutedUsers");
                                                                await message.reply(`🫂 ${targetName} has been unmuted.`);
                                                        } else {
                                                                await message.reply(`ℹ️ ${targetName} is not muted.`);
                                                        }
                                                        log.info("REACT_BY", `[ADDUSER] ${senderID} unmuted ${targetID} in ${threadID}`);
                                                } catch (err) {
                                                        log.err("REACT_BY", "Failed to unmute user", err);
                                                }
                                                return;
                                        }
                                }
