...
function patchFile(filePath, patches) {
    if (!fs.existsSync(filePath)) {
        console.log(`mostakim-v2-fca: ${path.basename(filePath)} not found, skipping.`);
        return;
    }
    ...
    if (changed) {
        ...
        console.log(`mostakim-v2-fca: Successfully patched ${path.basename(filePath)}.`);
    } else {
        console.log(`mostakim-v2-fca: ${path.basename(filePath)} is already clean, no changes required.`);
    }
}

const srcDir = path.join(__dirname, "node_modules", "@mostakim-v2", "fca-unofficial", "src");
...
console.log("mostakim-v2-fca: Patch analysis completed successfully.");
